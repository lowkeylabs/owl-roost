# src/owlroost/metrics/plugins/owl_outputs.py
#
# Copyright (c) 2026 John Leonard
# SPDX-License-Identifier: GPL-3.0-or-later
# See LICENSE file in repository root.

"""
TODO: Document module.

Notes
-----
Describe responsibilities, ownership,
and architectural role.

Canonical metrics model:

    - modeled outcomes
    - analytical projections
    - aggregate reductions

"""

from __future__ import annotations

from typing import TypedDict

from owlroost.metrics.specs import (
    MetricSpec,
)

from ...catalog.ontology import (
    AnalyticKind,
    CatalogNodeType,
    MaterializationLevel,
    Owner,
    ProjectionKind,
    SemanticDomain,
    ValueOrigin,
)

# =========================================================
# Canonical Metrics
# =========================================================

#
# IMPORTANT
# =========
#
# These are the canonical semantic metric
# definitions.
#
# This module intentionally owns ONLY:
#
#     - metric ontology bootstrap
#     - aggregate ontology synthesis
#
# Display overlays belong downstream
# in the display subsystem.
#
# Long-term this list may become:
#
#     - autogenerated
#     - coverage validated
#     - synchronized against metrics.json
#     - synthesized from builders.py
#
# =========================================================


class MetricOntology(
    TypedDict,
    total=False,
):
    owner: Owner
    semantic_domain: SemanticDomain
    value_origin: ValueOrigin
    projection_kind: ProjectionKind
    analytic_kind: AnalyticKind
    materialization_level: MaterializationLevel
    node_type: CatalogNodeType


OWL_DECISION_OUTPUT = MetricOntology(
    owner="OWL",
    semantic_domain="decision",
    value_origin="owl-computed",
    projection_kind="canonical",
    analytic_kind="primary",
    materialization_level="trial",
)

ROOST_EXECUTION_OUTPUT = MetricOntology(
    owner="ROOST",
    semantic_domain="execution",
    value_origin="roost-computed",
    projection_kind="canonical",
    analytic_kind="primary",
)


CANONICAL_METRICS: list[MetricSpec] = [
    # =====================================================
    # Timing
    # =====================================================
    MetricSpec(
        name="timing.elapsed_seconds",
        description=("Elapsed execution time for a single trial."),
        **ROOST_EXECUTION_OUTPUT,
        materialization_level="trial",
        dtype=float,
        aggregatable=True,
        default_aggregates=[
            "mean",
            "median",
            "p90",
            "max",
        ],
    ),
    # =====================================================
    # Trial Inventory
    # =====================================================
    MetricSpec(
        name="trial.completed",
        description=("Number of completed trials."),
        **ROOST_EXECUTION_OUTPUT,
        materialization_level="run",
        dtype=int,
        aggregatable=False,
    ),
    MetricSpec(
        name="trial.pending",
        description=("Number of pending trials."),
        **ROOST_EXECUTION_OUTPUT,
        materialization_level="run",
        dtype=int,
        aggregatable=False,
    ),
    MetricSpec(
        name="trial.total",
        description=("Total number of trials."),
        owner="ROOST",
        semantic_domain="design",
        value_origin="roost-computed",
        projection_kind="canonical",
        analytic_kind="primary",
        materialization_level="run",
        dtype=int,
        aggregatable=False,
    ),
    MetricSpec(
        name="trial.completion_rate",
        category="metric",
        description=("Fraction of completed trials."),
        owner="ROOST",
        semantic_domain="execution",
        value_origin="roost-computed",
        projection_kind="synthetic",
        analytic_kind="primary",
        materialization_level="run",
        dtype=float,
        aggregatable=False,
        derived_from=[
            "trial.completed",
            "trial.total",
        ],
    ),
    # =====================================================
    # Financial Outcomes
    # =====================================================
    MetricSpec(
        name="financial.spending.year0.today",
        category="metric",
        description=("Inflation-adjusted year-0 spending."),
        dtype=float,
        aggregatable=True,
        default_aggregates=[
            "mean",
            "median",
            "p10",
            "p90",
        ],
        **OWL_DECISION_OUTPUT,
    ),
    MetricSpec(
        name="financial.spending.year5.today",
        category="metric",
        description=("Inflation-adjusted year-5 spending."),
        dtype=float,
        aggregatable=True,
        default_aggregates=[
            "mean",
            "median",
            "p10",
            "p90",
        ],
        **OWL_DECISION_OUTPUT,
    ),
    MetricSpec(
        name="financial.spending.year10.today",
        category="metric",
        description=("Inflation-adjusted year-10 spending."),
        dtype=float,
        aggregatable=True,
        default_aggregates=[
            "mean",
            "median",
            "p10",
            "p90",
        ],
        **OWL_DECISION_OUTPUT,
    ),
    MetricSpec(
        name="financial.spending.total.today",
        description=("Total lifetime inflation-adjusted spending."),
        **OWL_DECISION_OUTPUT,
        dtype=float,
        aggregatable=True,
        default_aggregates=[
            "mean",
            "median",
            "p10",
            "p90",
        ],
    ),
    MetricSpec(
        name="financial.taxes.total.today",
        description=("Total lifetime inflation-adjusted taxes."),
        **OWL_DECISION_OUTPUT,
        dtype=float,
        aggregatable=True,
        default_aggregates=[
            "mean",
            "median",
            "p10",
            "p90",
        ],
    ),
    MetricSpec(
        name="financial.taxes.year0.today",
        description=("First year taxes paid."),
        **OWL_DECISION_OUTPUT,
        dtype=float,
        aggregatable=True,
        default_aggregates=[
            "mean",
            "median",
            "p10",
            "p90",
        ],
    ),
    MetricSpec(
        name="financial.roth.annual.year0.today",
        description=("Roth conversions for year 0."),
        **OWL_DECISION_OUTPUT,
        dtype=float,
        aggregatable=True,
        default_aggregates=[
            "mean",
            "median",
            "p10",
            "p90",
        ],
    ),
    MetricSpec(
        name="financial.roth.cumulative.terminal.today",
        description=("Total lifetime dollars converted to Roth."),
        **OWL_DECISION_OUTPUT,
        dtype=float,
        aggregatable=True,
        default_aggregates=[
            "mean",
            "median",
            "p10",
            "p90",
        ],
    ),
    MetricSpec(
        name="financial.bequest.terminal.today",
        description=("Terminal inflation-adjusted bequest."),
        **OWL_DECISION_OUTPUT,
        dtype=float,
        aggregatable=True,
        default_aggregates=[
            "mean",
            "median",
            "p10",
            "p90",
        ],
    ),
    MetricSpec(
        name="social_security.ages",
        description=("Output social security ages.  May be optimized. See withSSAges."),
        **OWL_DECISION_OUTPUT,
        dtype=list,
        # Vector-valued metric.
        # Aggregates are applied element-wise:
        #
        #     mean((x,y))
        #         ->
        #     (mean(x), mean(y))
        #
        aggregatable=True,
        default_aggregates=[
            "mean",
            "median",
            "p10",
            "p90",
        ],
    ),
    MetricSpec(
        name="social_security.optimized",
        description=("Are output social_security.ages optimized? See withSSAges."),
        **OWL_DECISION_OUTPUT,
        dtype=bool,
        aggregatable=True,
        default_aggregates=["constant"],
    ),
    # =====================================================
    # Risk
    # =====================================================
    MetricSpec(
        name="risk.summary.overall_risk",
        category="metric",
        description=("Overall scenario risk classification."),
        owner="OWL",
        semantic_domain="decision",
        value_origin="owl-computed",
        projection_kind="synthetic",
        analytic_kind="primary",
        materialization_level="trial",
        dtype=str,
        aggregatable=False,
    ),
    MetricSpec(
        name="risk.summary.scenario_severity",
        category="metric",
        description=("Severity score for scenario outcomes."),
        owner="OWL",
        semantic_domain="decision",
        value_origin="owl-computed",
        projection_kind="synthetic",
        analytic_kind="primary",
        materialization_level="trial",
        dtype=float,
        aggregatable=True,
        default_aggregates=[
            "mean",
            "median",
            "p90",
        ],
    ),
]

# =========================================================
# Plugin
# =========================================================


class OutputMetricsPlugin:
    """
    Register canonical output metrics.

    Ontology semantics live directly
    inside MetricSpec via:

        OntologySpec

    This plugin intentionally owns ONLY:

        - metric ontology bootstrap
        - aggregate ontology synthesis

    Canonical metrics model:

        - direct runtime observations
        - synthetic analytical helpers
        - aggregate analytical reductions
    """

    def register(
        self,
        registry,
    ):
        # =================================================
        # Canonical Metrics
        # =================================================

        for metric in CANONICAL_METRICS:
            registry.register(metric)

        # =================================================
        # Aggregate Metrics
        # =================================================

        canonical_metrics = list(registry.all())

        for metric in canonical_metrics:
            if not metric.aggregatable:
                continue

            for aggregate_name in metric.default_aggregates:
                aggregate_metric_name = f"{metric.name}__{aggregate_name}"

                registry.register(
                    MetricSpec(
                        # ---------------------------------
                        # Identity
                        # ---------------------------------
                        name=aggregate_metric_name,
                        category="aggregate",
                        description=(f"{aggregate_name} aggregate of {metric.name}"),
                        # ---------------------------------
                        # Ontology
                        # ---------------------------------
                        owner=metric.owner,
                        semantic_domain=(metric.semantic_domain),
                        value_origin=(metric.value_origin),
                        projection_kind="aggregate",
                        analytic_kind="aggregate",
                        materialization_level="run",
                        # ---------------------------------
                        # Provenance
                        # ---------------------------------
                        defined_in=("metrics/plugins/output_metrics.py"),
                        derived_from=[
                            metric.name,
                        ],
                        aggregate_function=(aggregate_name),
                        # ---------------------------------
                        # Typing
                        # ---------------------------------
                        dtype=metric.dtype,
                        # ---------------------------------
                        # Aggregation
                        # ---------------------------------
                        aggregatable=False,
                        default_aggregates=[],
                        # ---------------------------------
                        # Notes
                        # ---------------------------------
                        notes=("Autogenerated aggregate metric."),
                    )
                )
